...
from data_loader_err_detection import CARLA_Data
...
parser.add_argument('--model-name', type=str, default='cycle_gan', help='Directory to log data to.')
parser.add_argument('--data-dir', type=str, default='./data', help='Directory to dataset')

parser.add_argument('--margin', type=float, default=0.4, help='Triplet loss margin')
parser.add_argument("--perception-type", type=str, default='cam', help="perception input: cam | pred-sem | gt-sem | bev")
parser.add_argument("--use-lidar", default=False, action='store_true', help="use lidar as input")
parser.add_argument("--use-sem-image", default=False, action='store_true', help="use sem image as target (else bev encoding as target)")
parser.add_argument("--use-DA-sem", default=False, action='store_true', help="use DA sem image")
    ...
    def train(self):
        ...
        for data in tqdm(dataloader_train):
            ...
            if args.use_sem_image:
                sem_image_gt_fronts_in = data['sem_image_gt_fronts']
                sem_image_gt_lefts_in = data['sem_image_gt_lefts']
                sem_image_gt_rights_in = data['sem_image_gt_rights']
                sem_image_err_fronts_in = data['sem_image_err_fronts']
                sem_image_err_lefts_in = data['sem_image_err_lefts']
                sem_image_err_rights_in = data['sem_image_err_rights']
                sem_image_gt_fronts = []
                sem_image_gt_lefts = []
                sem_image_gt_rights = []
                ...
                sem_image_gt_fronts.append(sem_image_gt_fronts_in[0].to(args.device, dtype=torch.float32))
                sem_image_err_fronts.append(sem_image_err_fronts_in[0].to(args.device, dtype=torch.float32))
                if not args.ignore_sides:
                    sem_image_gt_lefts.append(sem_image_gt_lefts_in[0].to(args.device, dtype=torch.float32))
                    sem_image_gt_rights.append(sem_image_gt_rights_in[0].to(args.device, dtype=torch.float32))
                    sem_image_err_lefts.append(sem_image_err_lefts_in[0].to(args.device, dtype=torch.float32))
                    sem_image_err_rights.append(sem_image_err_rights_in[0].to(args.device, dtype=torch.float32))

                data_input['positive_input'] = sem_image_gt_fronts + sem_image_gt_lefts + sem_image_gt_rights
                data_input['negative_input'] = sem_image_err_fronts + sem_image_err_lefts + sem_image_err_rights
            else:
                data_input['positive_input'] = [data['graph_feature_gts'][0].to(args.device, dtype=torch.float32)]
                data_input['negative_input'] = [data['graph_feature_errs'][0].to(args.device, dtype=torch.float32)]

            pred_feature, positive_feature, negative_feature = model(data_input)

            margin = args.margin
            triplet_loss_fn = nn.TripletMarginWithDistanceLoss(
                distance_function=lambda x, y: 1.0 - F.cosine_similarity(x, y), margin=margin)
            triplet_loss = triplet_loss_fn(pred_feature, positive_feature, negative_feature)
            
            #graph_loss = graph_loss_with_logits(pred_graph_feature, target_graph_feature).mean()
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for batch_num, data in enumerate(tqdm(dataloader_val), 0):
                ...
                wp_epoch += float(triplet_loss.item())
                num_batches += 1
