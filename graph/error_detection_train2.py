...
class Trainer(object):
    ...
    def train(self):
        ...
        for data in tqdm(dataloader_train):
            ...
            positive_feature, negative_feature = model(data_input)
            prediction = torch.cat([positive_feature, negative_feature], dim=0)
            target = torch.Tensor([1]*positive_feature.shape[0] + [0]*negative_feature.shape[0]).to(args.device).unsqueeze(1)
