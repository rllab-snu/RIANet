...
class Model(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        self.feature_generator = models.resnet34()
        num_input_channel = sum(self.base_channel_list) + sum(self.base_channel_list2)
        self.feature_generator.conv1 = nn.Conv2d(num_input_channel, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.feature_generator.fc = nn.Linear(512, 1, bias=False)
    ...
    def forward(self, data):
        ...
        positive_feature = torch.cat(input_ + positive_input_list, dim=1) if positive_input_list is not None else None
        positive_feature = self.feature_generator(positive_feature)
        
        negative_feature = torch.cat(input_ + negative_input_list, dim=1) if negative_input_list is not None else None
        negative_feature = self.feature_generator(negative_feature)
