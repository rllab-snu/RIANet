...
class DLA(nn.Module):
    def __init__(self, levels, channels, base_channel=3, num_classes=1000,
        ...
        self.base_layer = nn.Sequential(
            nn.Conv2d(base_channel, channels[0], kernel_size=7, stride=1,
...
class DLASeg(nn.Module):
    def __init__(self, base_name, classes, base_channel_list=[3,2],
                 pretrained_base=None, down_ratio=2):
        super(DLASeg, self).__init__()
        assert down_ratio in [2, 4, 8, 16]
        self.first_level = int(np.log2(down_ratio))
        self.fusion_channel = 8
        
        for i, base_channel in enumerate(base_channel_list):
            conv = nn.Sequential(
                        nn.Conv2d(base_channel, self.fusion_channel, kernel_size=7, stride=1,
                                  padding=3, bias=False),
                        BatchNorm(self.fusion_channel),
                        nn.ReLU(inplace=True),
                        nn.Conv2d(self.fusion_channel, self.fusion_channel, kernel_size=7, stride=1,
                                  padding=3, bias=False),
                        BatchNorm(self.fusion_channel),
                        nn.ReLU(inplace=True)
                        )            
        ...
        self.base = globals()[base_name](base_channel=self.fusion_channel * len(base_channel_list),
    ...
    def forward(self, x_list):
        ...
        for i, x_i in enumerate(x_list):
            x.append(getattr(self,'conv'+str(i))(x_i))
        x = torch.cat(x, dim=1)
        x = self.base(x)
...
class Model(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        if self.use_lidar:
            self.base_channel_list = [3, 2]
        else:
            self.base_channel_list = [3]

        self.encoder = globals()[self.config.model_name](self.config.feature_size, \
                                                        down_ratio=2, base_channel_list=self.base_channel_list)
        self.encoder = self.encoder.to(self.device)

    def forward(self, data):
        image_list, lidar_list = data['images'], data['lidars']
        '''
