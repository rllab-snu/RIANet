...
class RLAgent(AutonomousAgent):
    ...
    def input_data_process(self, input_data, timestamp):
        ...
        if not hasattr(self, 'prev_hdmap'):  # need to be regenerated for new map
            ...
            if self.args.localization == 'filter':
                self.localization = LocalizationOperator()
    ...
    def run_step(self, input_data, timestamp):
        ...
        vehicle_gt_dev = vehicle_gt_st_partial.squeeze()[1]

        brake, obstacles, redlight = self._should_brake()
        vehicle_obstacles = np.array(obstacles[0])
        walker_obstacles = np.array(obstacles[1])

        env_data = {'image_data': self.image_data, 'lidar_data': self.lidar_data,
                    'lidar_transformed_0': self.lidar_transformed_0, 'lidar_transformed_1': self.lidar_transformed_1,
                    'gnss_data': self.gnss_data, 'compass_data': self.compass_data, 'speed_data': self.speed_data,
                    'vehicle_pos': vehicle_pos_xy, 'vehicle_gt_pos': self.vehicle_gt_pos,
                    'vehicle_gt_dev': vehicle_gt_dev,
                    'G2': self.G2, 'torch_G2': self.torch_G2, 'pos_kdtree_keys': self.pos_kdtree_keys,
                    'pos_kdtree': self.pos_kdtree,
                    'global_traj': self.global_traj, 'global_traj_node_id': self.global_traj_node_id,
                    'global_plan_pos': self.global_plan_pos,
                    'vehicle_obstacles':vehicle_obstacles, 'walker_obstacles':walker_obstacles, 'redlight':redlight,
                    'scenario_manager':self.scenario_manager}

        # get high-level control
        high_level_control = self.RL_Trainer.update_virtual_env_and_get_control(env_data)
        steer_adjustment, target_speed = high_level_control[0] + self.front_target_lane_dev, high_level_control[1]
        ...
        self.data_history['env_info_dict'].append(env_info_dict)
    ...
    def destroy(self):
        ...
        self.RL_Trainer.reset_scenario_and_state()
        ...
        if hasattr(self, 'data_history'):
            ...
            pos_err_history = np.linalg.norm(self.data_history['pos_gt'] - self.data_history['pos'], axis=1)
    ...
    def _should_brake(self):
        ...
        return any([vehicle, walker, redlight]), (vehicle_xy_list, walker_xy_list), redlight
