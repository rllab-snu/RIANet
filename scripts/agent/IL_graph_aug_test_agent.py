...
class ILAgent(AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        if self.args.use_error_detection:
            with open(os.path.join(self.args.model_dir, 'gac_2d_detection_self_attention/args.txt')) as f:
                d = json.load(f)
            ...
            if self.args.test_last_model:
                self.net.load_state_dict(
                    torch.load(os.path.join(self.args.model_dir, 'gac_2d_detection_self_attention/model_101.pth'), map_location=self.device))
            else:
                self.net.load_state_dict(
                    torch.load(os.path.join(self.args.model_dir, 'gac_2d_detection_self_attention/best_model.pth'), map_location=self.device))
            self.net.to(self.device)
            self.net.eval()

            with open(os.path.join(self.args.model_dir, 'gac_2d_detection_no_graph/args.txt')) as f:
                d = json.load(f)
                self.model_args_no_graph = namedtuple('args', d.keys())(*d.values())
            self.net_no_graph = importlib.import_module('IL.models.' + self.model_args_no_graph.model_name).Model(self.model_args_no_graph,
                                                                                                  self.device)
            if self.args.test_last_model:
                self.net_no_graph.load_state_dict(
                    torch.load(os.path.join(self.args.model_dir, 'gac_2d_detection_no_graph/model_101.pth'), map_location=self.device))
            else:
                self.net_no_graph.load_state_dict(
                    torch.load(os.path.join(self.args.model_dir, 'gac_2d_detection_no_graph/best_model.pth'), map_location=self.device))
            self.net_no_graph.to(self.device)
            self.net_no_graph.eval()
        else:
        ...
        if self.args.record_video:
            ...
            (self.save_path / 'sem').mkdir(parents=True, exist_ok=False)
            (self.save_path / 'pred_sem').mkdir(parents=True, exist_ok=False)
    ...
    def graph_updater_thread(self):
        ...
        while (len(self.processed_roads) < self.num_of_roads):
            ...
            global_traj_prev_node_id = global_traj_node_id.copy()
            seg_name_list = list(set(['_'.join(node_id.split('_')[:2]) for node_id in global_traj_node_id]))
            seg_name_list = [seg for seg in seg_name_list \
                             if seg_G.has_node(seg) and seg_G.nodes[seg]['on_traffic_signal'] is None]
            road_name_list = []
            new_seg_name_list = []
            for seg in seg_name_list:
                if seg.split('_')[0] not in road_name_list:
                    road_name_list.append(seg.split('_')[0])
                    new_seg_name_list.append(seg)
            if len(new_seg_name_list) < 3:
                sampled_seg_name_list = new_seg_name_list
            else:
                sampled_seg_name_list = random.sample(new_seg_name_list, 3)
            print('rearrange start', sampled_seg_name_list)

            G_arrange, seg_G2, error_result = route_augment(G2.copy(), seg_G.copy(), sampled_seg_name_list)
            ...
            # get new global_traj
            global_traj, global_traj_node_id, path_progress, (plan_start_idx, plan_end_idx) = \
                get_global_traj_with_error(self.global_plan_pos_with_starting, G_arrange, seg_G2, \
                                           pos_kdtree_keys_arrange, pos_kdtree_arrange, G, seg_G)
            ...
            self.G_arrange, self.seg_G2, self.pos_kdtree_keys_arrange, self.pos_kdtree_arrange = \
                G_arrange, seg_G2, pos_kdtree_keys_arrange, pos_kdtree_arrange
            self.global_traj_prev_node_id = global_traj_prev_node_id
    ...
    def tick(self, input_data, timestamp):
        ...
        if not hasattr(self, 'prev_hdmap'):  # need to be regenerated for new map
            ...
            self.global_traj_prev_node_id = self.global_traj_node_id.copy()
            seg_name_list = list(set(['_'.join(node_id.split('_')[:2]) for node_id in self.global_traj_node_id]))
            seg_name_list = [seg for seg in seg_name_list \
                             if self.seg_G.has_node(seg) and self.seg_G.nodes[seg]['on_traffic_signal'] is None]
            road_name_list = []
            new_seg_name_list = []
            for seg in seg_name_list:
                if seg.split('_')[0] not in road_name_list:
            ...
            if len(new_seg_name_list) < 3:
                ...
                sampled_seg_name_list = random.sample(new_seg_name_list, 3)
            print('rearrange start', sampled_seg_name_list)
            self.G_arrange, self.seg_G2, self.error_result = route_augment(self.G2.copy(), self.seg_G.copy(), sampled_seg_name_list)
            self.pos_kdtree_keys_arrange = np.stack(list(dict(self.G_arrange.nodes('pos')).keys()))
            self.pos_kdtree_arrange = KDTree(np.stack(list(dict(self.G_arrange.nodes('pos')).values())))

            # get new global_traj
            self.global_traj, self.global_traj_node_id, self.path_progress, (plan_start_idx, plan_end_idx) = \
                get_global_traj_with_error(self.global_plan_pos_with_starting, self.G_arrange, self.seg_G2, \
                                self.pos_kdtree_keys_arrange, self.pos_kdtree_arrange, self.G, self.seg_G)
            print('rearrange done')

            # build the model from a config file and a checkpoint file
            config_file = '/home/timothyha/Research/mmsegmentation/work_dirs/deeplabv3plus_r50-d8_300x400_80k_carla_DA/deeplabv3plus_r50-d8_300x400_80k_carla_DA.py'
            checkpoint_file = '/home/timothyha/Research/mmsegmentation/work_dirs/deeplabv3plus_r50-d8_300x400_80k_carla_DA/latest.pth'

            self.semantic_model = init_segmentor(config_file, checkpoint_file, device=self.device)
            self.semantic_model.CLASSES = color_code_DA.keys()
            self.semantic_model.PALLETE = [[int(w) for w in c[0]] for c in color_code_DA.values()]
            self.coord_converters = [PC_CoordConverter(cam_yaw=yaw) for yaw in [0]]
        ...
        graph_feature_dict, H = extract_graph_features(self.vehicle_pos[:2], 0.5 * np.pi - compass, speed, \
                                                    self.G_arrange, self.pos_kdtree_keys_arrange, self.pos_kdtree_arrange, self.global_traj_node_id, \
                                                    nearest_node_num=96, use_node_filter=True)

        sem_img = self.coord_converters[0].visualize_sem_image(rgb, self.seg_G2, H, graph_feature_dict, \
                                                              self.vehicle_pos[:2], 0.5 * np.pi - compass, stride=60)
        sem_img = np.clip(sem_img.argmax(-1), 0, 3)

        result = inference_segmentor(self.semantic_model, rgb)
        pred_sem_img = np.eye(4)[result[0]]
        pred_sem_img = np.clip(pred_sem_img.argmax(-1), 0, 3)
    ...
    def run_step(self, input_data, timestamp):
        ...
        # check lane error
        if not hasattr(self, 'has_lane_error'):
            self.has_lane_error = False
        elif self.step % 5 == 0:
            _, self.mIoU = get_seg_accuracy(tick_data['sem_img'], tick_data['pred_sem_img'], num_class=4)
            self.has_lane_error = (self.mIoU < 0.46)
        ...
        if (self.step % 2 == 0 or self.step <= 4):
            ...
            if hasattr(self.net.config, "pred_target_point") and self.net.config.pred_target_point:
                if self.args.use_error_detection and self.has_lane_error:
                    self.pred_wp = self.net_no_graph(torch_data)
                else:
        ...
        if hasattr(self.net.config, "pred_target_point") and self.net.config.pred_target_point:
            if self.args.use_error_detection:
                if not self.has_lane_error:
                    ...
                    steer, _ = self.pid_controller.control(vehicle_pos_xy, 0.5 * np.pi - tick_data['compass'],
                                                           tick_data['speed'],
                                                           vehicle_st_partial, target_xy,
                                                           target_speed=target_speed, steer_adjustment=steer_adjustment)
                else:
                    steer, throttle, brake, metadata = self.net_no_graph.control_pid(self.pred_wp, gt_velocity)
                    self.pid_metadata = metadata
            else:
    ...
    def save(self, tick_data):
        ...
        Image.fromarray(sem_to_rgb_DA(tick_data['sem_img'])).save(self.save_path / 'sem' / ('%04d.png' % frame))
        Image.fromarray(sem_to_rgb_DA(tick_data['pred_sem_img'])).save(self.save_path / 'pred_sem' / ('%04d.png' % frame))
    ...
    def destroy(self):
        ...
        np.save(self.save_path / 'error_data.npy', self.error_result)
        np.save(self.save_path / 'global_traj_prev_data.npy', self.global_traj_prev_node_id)
        np.save(self.save_path / 'global_traj_data.npy', self.global_traj_node_id)
