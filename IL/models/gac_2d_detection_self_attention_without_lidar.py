...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.visual_pos_emb = nn.Parameter(torch.zeros(1, 2+2*self.config.max_obj_num+self.config.max_node_num, self.config.f_out))
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num,
        ...
        visual_features = torch.stack([image_features, velocity_features], dim=1)
        visual_features = torch.cat([visual_features, td_obj_features, od_obj_features, graph_features], dim=1)  # batch x (3+obj+obj) x f
        visual_features += self.visual_pos_emb
        one_mask = torch.ones([bz,2]).to(device=visual_features.device, dtype=torch.int64)
        visual_feature_mask = torch.cat([one_mask, td_mask, od_mask, graph_feature_mask], dim=1)
