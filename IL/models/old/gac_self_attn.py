...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.self_attn_transformer1 = Attblock(self.config.f_out,
                                      self.config.n_head,
                                      self.config.dim_feedforward,
                                      self.config.dropout)
        self.self_attn_transformer2 = Attblock(self.config.f_out,
                                      self.config.n_head,
                                      self.config.dim_feedforward,
                                      self.config.dropout)
        self.self_attn_transformer3 = Attblock(self.config.f_out,
                                      self.config.n_head,
                                      self.config.dim_feedforward,
                                      self.config.dropout)
        self.self_attn_transformer4 = Attblock(self.config.f_out,
                                      self.config.n_head,
                                      self.config.dim_feedforward,
                                      self.config.dropout)
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num):
        ...
        total_features = torch.cat([visual_features, graph_features], dim=1)

        feature_mask_visual = torch.ones([node_num.shape[0], 3]).to(dtype=torch.int64, device=feature_mask.device)
        feature_mask_total = torch.cat([feature_mask, feature_mask_visual], dim=1)
        
        total_features, total_attn = self.self_attn_transformer1(total_features, total_features, feature_mask_total)  # batch x (k + n) x f
        total_features, total_attn = self.self_attn_transformer2(total_features, total_features, feature_mask_total)  # batch x (k + n) x f
        total_features, total_attn = self.self_attn_transformer3(total_features, total_features, feature_mask_total)  # batch x (k + n) x f
        total_features, total_attn = self.self_attn_transformer4(total_features, total_features, feature_mask_total)  # batch x (k + n) x f
        ...
        fused_features = (total_features.sum(dim=1) / (feature_mask_total.sum(dim=1).unsqueeze(1) + 1e-6))
