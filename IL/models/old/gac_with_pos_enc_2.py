...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.visual_pos_emb = nn.Parameter(
            torch.zeros(1, 3, self.config.f_out))
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num):
        ...
        visual_features = torch.stack([image_features, lidar_features, velocity_features], dim=1)  # batch x k x f
        visual_features = visual_features + self.visual_pos_emb
