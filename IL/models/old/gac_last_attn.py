...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.last_attn_layer1 = nn.Linear(self.config.f_out, 1)
        self.last_attn_layer2 = nn.Linear(self.config.f_out, 1)
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num):
        ...
        last_attn1 = F.softmax(self.last_attn_layer1(visual_context), dim=1)  # batch x k x 1
        last_attn2 = F.softmax(self.last_attn_layer2(graph_context), dim=1)  # batch x node x 1

        visual_context = (visual_context * last_attn1).sum(dim=1)  # batch x f
        graph_context = (graph_context * last_attn2).sum(dim=1)  # batch x f
