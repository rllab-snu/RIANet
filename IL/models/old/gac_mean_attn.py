...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num):
        ...
        visual_mean = visual_context.mean(dim=1).unsqueeze(1)  # batch x 1 x f
        visual_context, _ = self.last_attn_transformer1(visual_mean, visual_context, None)  # batch x 1 x f
        graph_mean = (graph_context.sum(dim=1) / (feature_mask.sum(dim=1).unsqueeze(1) + 1e-6)).unsqueeze(1)  # batch x 1 x f
        graph_context, _ = self.last_attn_transformer2(graph_mean, graph_context, feature_mask)  # batch x 1 x f

        fused_features = torch.cat([visual_context.squeeze(1), graph_context.squeeze(1)], -1)   # batch x (2 * f)
