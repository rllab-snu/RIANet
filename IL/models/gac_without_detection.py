...
class Object_encoder(nn.Module):
    def __init__(self, c_size, f_size, max_obj_num=12):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(8, c_size, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.BatchNorm2d(c_size), nn.ReLU(),
                                   nn.Conv2d(c_size, c_size, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.BatchNorm2d(c_size), nn.ReLU(),
                                   nn.Conv2d(c_size, c_size, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.BatchNorm2d(c_size))
        self.linear1 = nn.Linear(c_size, f_size)

        # self.img_pos_emb = nn.Parameter(torch.zeros(1, 1, 1, 256, 256))
        self.img_pos_emb = nn.Parameter(torch.zeros(1, 1, 256, 256))

    def forward(self, img_tensor, img_class, img_mask, obj_num):
        # img_tensor: b x 3 x 256 x 256
        # img_class: b x obj x 4
        # img_mask: b x obj x 256 x 256
        # obj_num: b
        bz = img_tensor.shape[0]

        img_class_expand = img_class.unsqueeze(3).unsqueeze(4).expand(-1, -1, -1, 256,
                                                                      256)  # b x obj x 4 -> b x obj x 4 x 256 x 256
        img_classification = (img_class_expand * img_mask.unsqueeze(2)).max(1).values  # batch x 4 x 256 x 256
        # img_patch = torch.cat([img_tensor, img_classification], dim=1)  # b x 7 x 256 x 256
        img_patch = torch.cat([img_tensor, img_classification, self.img_pos_emb.expand(bz, -1, -1, -1)],
                              dim=1)  # b x 8 x 256 x 256

        # conv_feature = self.conv1(img_patch) + self.img_pos_emb
        conv_feature = self.conv1(img_patch).unsqueeze(1).expand(-1, self.max_obj_num, -1, -1,
                                                                 -1)  # b x obj x c x 256 x 256
        conv_feature = conv_feature * img_mask.unsqueeze(2)

        conv_feature_sum = conv_feature.sum(dim=4).sum(dim=3)  # b x obj x c
        # conv_feature_mean = conv_feature_sum / (img_mask.sum(dim=3).sum(dim=2).unsqueeze(2) + 1e-6)
        final_obj_feature = self.linear1(conv_feature_sum)  # b x obj x f

        obj_feature_mask = 1 - torch.cumsum(F.one_hot(obj_num, num_classes=self.max_obj_num + 1), dim=1)[:,
                               :-1]  # b x obj
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.visual_pos_emb = nn.Parameter(torch.zeros(1, 3, self.config.f_out))
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num):
        ...
        graph_feature_mask = 1 - torch.cumsum(F.one_hot(node_num, num_classes=self.config.max_node_num + 1), dim=1)[:,
                                 :-1]
        ...
        visual_feature_mask = torch.ones([bz, 3]).to(device=visual_features.device, dtype=torch.int64)

        visual_context, visual_attn = self.visual_query_transformer1(visual_features, graph_features,
                                                                     graph_feature_mask)  # batch x k x f
        graph_context, graph_attn = self.graph_query_transformer1(graph_features, visual_features,
                                                                  visual_feature_mask)  # batch x node x f

        visual_context, visual_attn = self.visual_query_transformer2(visual_context, graph_context,
                                                                     graph_feature_mask)  # batch x k x f
        graph_context, graph_attn = self.graph_query_transformer2(graph_context, visual_context,
                                                                  visual_feature_mask)  # batch x node x f

        visual_context, visual_attn = self.visual_query_transformer3(visual_context, graph_context,
                                                                     graph_feature_mask)  # batch x k x f
        graph_context, graph_attn = self.graph_query_transformer3(graph_context, visual_context,
                                                                  visual_feature_mask)  # batch x node x f

        visual_context, visual_attn = self.visual_query_transformer4(visual_context, graph_context,
                                                                     graph_feature_mask)  # batch x k x f
        graph_context, graph_attn = self.graph_query_transformer4(graph_context, visual_context,
                                                                  visual_feature_mask)  # batch x node x f

        visual_context_masked = visual_context * visual_feature_mask.unsqueeze(2)
        visual_mean = (
                    visual_context_masked.sum(dim=1) / (visual_feature_mask.unsqueeze(2).sum(dim=1) + 1e-6)).unsqueeze(
            1)  # batch x 1 x f
        visual_context, _ = self.last_attn_transformer1(visual_mean, visual_context,
                                                        visual_feature_mask)  # batch x 1 x f

        graph_context_masked = graph_context * graph_feature_mask.unsqueeze(2)
        graph_mean = (graph_context_masked.sum(dim=1) / (graph_feature_mask.sum(dim=1).unsqueeze(1) + 1e-6)).unsqueeze(
            1)  # batch x 1 x f
...
class Model(nn.Module):
    ...
    def forward(self, data):
        ...
        adj, node_feature_matrix, edge_feature_matrix = data['adjacency_matrix'], data['node_feature_matrix'], data[
            'edge_feature_matrix']
