...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.visual_pos_emb = nn.Parameter(torch.zeros(1, 3+self.config.max_node_num, self.config.f_out))
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num,
        ...
        visual_features = torch.cat([visual_features, graph_features], dim=1)  # batch x (3+obj+obj) x f
        visual_features += self.visual_pos_emb
        one_mask = torch.ones([bz,3]).to(device=visual_features.device, dtype=torch.int64)
        visual_feature_mask = torch.cat([one_mask, graph_feature_mask], dim=1)

        visual_context, visual_attn = self.visual_query_transformer1(visual_features, visual_features, visual_feature_mask)  # batch x k x f
        visual_context, visual_attn = self.visual_query_transformer2(visual_context, visual_context, visual_feature_mask)  # batch x k x f
        visual_context, visual_attn = self.visual_query_transformer3(visual_context, visual_context, visual_feature_mask)  # batch x k x f
        visual_context, visual_attn = self.visual_query_transformer4(visual_context, visual_context, visual_feature_mask)  # batch x k x f
        ...
        fused_features = visual_context.squeeze(1)
...
class Model(nn.Module):
    def __init__(self, args, device):
        ...
        self.join = nn.Sequential(
            nn.Linear(self.config.f_out, 256),
    ...
    def forward(self, data):
        ...
        td_img_class, td_img_mask, td_num = None, None, None #data['td_img_class'], data['td_img_mask'], data['td_num'].to(torch.int64)
        td_img_mask_info = None #data['td_img_mask_info']
        od_img_class, od_img_mask, od_num = None, None, None #data['od_img_class'], data['od_img_mask'], data['od_num'].to(torch.int64)
        od_img_mask_info = None #data['od_img_mask_info']
