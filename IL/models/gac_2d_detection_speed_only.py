...
class Object_encoder(nn.Module):
    ...
    def forward(self, img_tensor, img_class, img_mask, obj_num):
        ...
        img_class_expand = img_class.unsqueeze(3).unsqueeze(4).expand(-1, -1, -1, 256, 256)  # b x obj x 4 -> b x obj x 4 x 256 x 256
        ...
        img_patch = torch.cat([img_tensor, img_classification, self.img_pos_emb.expand(bz, -1, -1, -1)], dim=1)  # b x 8 x 256 x 256

        #conv_feature = self.conv1(img_patch) + self.img_pos_emb
        conv_feature = self.conv1(img_patch).unsqueeze(1).expand(-1,self.max_obj_num,-1,-1,-1)   # b x obj x c x 256 x 256
...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, velocity, adj, node_feature_matrix, edge_feature_matrix, node_num,
                td_img_class, td_img_mask, td_num, od_img_class, od_img_mask, od_num):
        # encoding
        ...
        td_obj_features, td_mask = self.td_encoder(image_tensor, td_img_class, td_img_mask, td_num)
        od_obj_features, od_mask = self.od_encoder(image_tensor, od_img_class, od_img_mask, od_num)
...
class Model(nn.Module):
    ...
    def forward(self, data):
        ...
        fused_features = self.encoder(image_list, lidar_list, velocity, \
                                      ...
                                      td_img_class, td_img_mask, td_num,
                                      od_img_class, od_img_mask, od_num)
